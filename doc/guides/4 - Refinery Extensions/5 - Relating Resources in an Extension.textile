...
Now, add a function to "find_all_event_types", and call that function in a before_action (we only need it in the show action).  Your controller should look like this when you are done:
  ...
  module Events
    module Admin
      class EventsController < ::Refinery::AdminController
        ...
        before_action :find_all_event_types, :only => [:show]
