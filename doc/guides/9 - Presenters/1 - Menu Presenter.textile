...
* configure and use +Refinery::MenuPresenter+
...
Now let's focus on the presenter itself. Once instantiated, it is also possible to configure its CSS/HTML using this instance. We will use a Rails helper to instantiate a new instance of +Refinery::MenuPresenter+ and also configure it there. We're taking this approach because we don't want to pollute the view with configuration code.
...
def footer_menu
  ...
  presenter = Refinery::MenuPresenter.new(menu_items, self)
